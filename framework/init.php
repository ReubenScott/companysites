...
class _init_app{
  ...
  function init_assign(){
    ...
    // 装载默认语言包
    if (is_file(ROOT . "data/xml/lang_default.xml")) {
      $this->lang = xml_to_array(file_get_contents(ROOT . "data/xml/lang_default.xml"));
    }
    // 加载语言包，如果存在
    if ($this->site ['lang'] && is_file(ROOT . "data/xml/langs/" . $this->site ['lang'] . ".xml")) {
      $lang = xml_to_array(file_get_contents(ROOT . "data/xml/langs/" . $this->site ['lang'] . ".xml"));
    }
    // 合并语言包
    if ($lang && $this->lang) {
      ...
      if ($site_rs ["tpl_id"]) {
        ...
        if ($rs) {
          ...
          if ($this->is_mobile) {
            ...
            if (! file_exists(ROOT . "tpl/" . $tplfolder)) {
...
class phpok_plugin extends _init_auto{
  ...
  function plugin(){
    ...
    $id = $this->plugin_id();
    $langs = "";
    if ($this->site ['lang'] && is_file(ROOT . 'plugins/' . $id . '/langs/' . $this->site ['lang'] . '.xml')) {
      $langs = xml_to_array(file_get_contents(ROOT . 'plugins/' . $id . '/langs/' . $this->site ['lang'] . '.xml'));
    } else {
      if (is_file(ROOT . 'plugins/' . $id . '/lang_default.xml')) {
        $langs = xml_to_array(file_get_contents(ROOT . 'plugins/' . $id . '/lang_default.xml'));
      }
    }
